...
extension TileGridView {
  ...
  fileprivate func startAnimatingWithBeginTime(_ beginTime: TimeInterval) {
    ...
    for tileRows in tileViewRows {
      for view in tileRows {
        ...
        vector = CGPoint(x: CGFloat(vector.x) * CGFloat(kRippleMagnitudeMultiplier) * CGFloat(distance), y: CGFloat(vector.y) * CGFloat(kRippleMagnitudeMultiplier) * CGFloat(distance))
  ...
  fileprivate func distanceFromCenterViewWithView(_ view: UIView)->Float {
    ...
    return Float(sqrt(normalizedX * normalizedX + normalizedY * normalizedY))
  }
  ...
  fileprivate func normalizedVectorFromCenterViewToView(_ view: UIView)->CGPoint {
    ...
    return CGPoint(x: deltaX / CGFloat(length), y: deltaY / CGFloat(length))
  }
}
